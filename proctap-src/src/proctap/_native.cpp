...
public:
    ...
    HRESULT InitializeForProcess(DWORD processId) {
        ...
        // Process Loopback (ActivateAudioInterfaceAsync with
        // AUDIOCLIENT_ACTIVATION_TYPE_PROCESS_LOOPBACK) is available on
        // Windows 10 2004 (build 19041) and later. The previous hard gate at
        // build 20438 was both wrong (a typo of 20348) and too restrictive:
        // OBS and others use this API on 19041+. We do NOT pre-gate here.
        // If activation is genuinely unsupported, ActivateAudioInterfaceAsync
        // will return a real HRESULT below and we fall back honestly then.
        OutputDebugStringA("INFO: Attempting process-specific capture via ActivateAudioInterfaceAsync (no version pre-gate)\n");
